...
zeruj_tabele($tabela);
oblicz($mecze,$tabela);
function oblicz($mecze,$tabela){
	$dana=mysql_query("SELECT * FROM $mecze WHERE wynik<>''");
	while ($r=mysql_fetch_array($dana)) {
		if ($r[4]==17||$r[5]==17) continue;
		$gosp=pobierz("*", "$tabela", "id", $r[id_h]);
		$gosc=pobierz("*", "$tabela", "id", $r[id_a]);
		$wynik=explode(":", $r[wynik]);
		if ($wynik[0]>$wynik[1]) {
			$gosp[1]+=1;
			$gosp[4]+=$wynik[0];
			$gosp[5]+=$wynik[1];
...
function zeruj_tabele($tabela){
	mysql_query("UPDATE $tabela SET z=0,r=0,p=0,bz=0,bs=0,z_h=0,r_h=0,
