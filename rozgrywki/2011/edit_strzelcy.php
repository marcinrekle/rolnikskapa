...
include '../../funkcje.php';
if ($_GET[ins]==1) {
	insert("strzelcy1112", "id,id_z,bk,pp,s", "null,'$_GET[id]','0','0','0'");
}
if ($_GET[id]<>'' && $_GET[roz]<>'') {
	$roz=$_GET[roz];
	$gracz=pobierz("*", "strzelcy1112", "id_z", $_GET[id]);
	if ($_GET[znak]==1) {
		$gracz[$roz]+=1;
	}else $gracz[$roz]-=1;
	update("strzelcy1112", "$roz=$gracz[$roz]", "id_z=$_GET[id]");
}
echo "<table>";
echo "<tr>"."\n".'<th>lp</th>'."\n".'<th>Zawodnik</th>'."\n".'<th>
...
$dane=mysql_query("SELECT * FROM strzelcy1112");
