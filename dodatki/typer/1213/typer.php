...
$dane=mysql_query("SELECT * FROM tab_typerow1213 ORDER by pkt DESC");
$m=0;
...
<div class="typ_wynik">Wyn</div><div class="typ_pkt">Pkt</div></div>';
...
while ($r=mysql_fetch_row($dane)) {
	...
	if ($c_m>time()&& $typ[3]<>NULL){
		$cmt='<a href="'.$c_m.'">'.$typ[3].'</a>';
	}
	elseif ($c_m<time()&& $typ[3]<>NULL) $cmt=$typ[3];elseif ($c_m>time()&& $typ[3]==NULL)
	$cmt='<a href="'.$c_m.'" >typuj</a>';elseif ($c_m<time()&& $typ[3]==NULL) $cmt='NT';
	if($c_m>time()){
...
<div id="loading" class="im" style="top:-50px;"><img src="../../../PLIKI/loading.gif" /></div>
</div>
</div>
