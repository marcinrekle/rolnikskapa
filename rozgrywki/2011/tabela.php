...
<div id="tresc">
     ...
     sieradzkiej B Klasy sezon 2011/12</div><div class="content">'."<table>";
     ...
     $s_rb=0;
     $s_b=0;
     $dana=mysql_query("SELECT * FROM tabela1112 ORDER by (z*3)+r DESC, bz-bs DESC,bz DESC");
     ...
     $s_rb=0;
     $s_b=0;
     $dana=mysql_query("SELECT * FROM tabela1112 ORDER by (z_h*3)+r_h DESC, bz_h-bs_h DESC,bz_h DESC");
     while ($r=mysql_fetch_row($dana)){
     	$i+=1;
     	$mecze=$r[6]+$r[7]+$r[8];
     	$pkt=$r[6]*3+$r[7];
     	...
     	<td>$r[9]</td><td>$r[9]-$r[10]</td><td>$rb</td><td></td></tr>";
     }//while
     ...
     $s_rb=0;
     $s_b=0;
     $dana=mysql_query("SELECT * FROM tabela1112 ORDER by (z_a*3)+r_a DESC, bz_a-bs_a DESC,bz_a DESC");
     while ($r=mysql_fetch_row($dana)){
     	$i+=1;
