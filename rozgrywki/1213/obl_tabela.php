...
function oblicz(){
	...
	$dana=mysql_query("SELECT * FROM mecze1213 WHERE wynik<>''");
	while ($r=mysql_fetch_row($dana)) {
		if ($r[4]==17||$r[5]==17) continue;
		$gosp=pobierz("*", "tabela1213", "id", $r[4]);
		$gosc=pobierz("*", "tabela1213", "id", $r[5]);
		$wynik=explode(":", $r[6]);
		if ($wynik[0]>$wynik[1]) {
			$gosp[1]+=1;
			$gosp[4]+=$wynik[0];
...
function zeruj_tabele(){
	mysql_query("UPDATE tabela1213 SET z=0,r=0,p=0,bz=0,bs=0,z_h=0,r_h=0,
