...
function sprawdz(href) {
	$(document).ready(
			function(){
				if($('.wyd_tlo')){
				var el1=$('.wyd_tlo');
				var wys=$('body :first-child').outerHeight(true); 
				var el2=$('.wyd_tytul');
				var el3 = $('.wyd_opis');
				var wys1 = 375 + wys;
				var wys2 = 380 + wys;
				var wys3 = 430 + wys;
				el1.attr("style","top:"+wys1+"px");
				el2.attr("style","top:"+wys2+"px");
				el3.attr("style","top:"+wys3+"px");
				}
				SbScroll('start');
			wyswietl(href);
			}
...
function wyswietl_cos(id) {
	...
	r.open('GET', 'rozgrywki/2011/obliczanie_tab.php?id=' + id + '&wynik='
			+ wynik, true);
	r.send(null);
...
function dodajBramke(gdzie, id, znak) {
	$(document).ready(
			function() {
				$.ajax( {
					url : "rozgrywki/2011/edit_strzelcy.php?id=" + id + "&roz="
							+ gdzie + "&znak=" + znak,
					success : function(data) {
						$("#editStrz").html(data);
					}
...
function dodajStrzelca(id) {
	$(document).ready(function() {
		$.ajax( {
			url : "rozgrywki/2011/edit_strzelcy.php?id=" + id + "&ins=1",
			success : function(data) {
				$("#editStrz").html(data);
			}
		});
...
function pokaz(adres2) {
	$(document).ready(function() {
					...
					$("body").append("<div id=\"panelRej\"></div>");
					$.ajax({
					url:'http://www.rolnikskapa.cba.pl/rejestracja_panel2.php',
					success:function(data){
						$("#panelRej").append(data);	
					}
					});
					//$("#panelRej").attr("name", adres2);	
	});
...
function SbEmoDod(emo){
	$(document).ready(
			function(){
				$("#SbMessage").append(emo);
			}
	);
}
function SbSent(){
	$(document).ready(
			function(){
				$.post('sb_dod.php',$("#SbFormMess").serialize(),
						function(){
					$("#SbMessage").html("Wyslano");
					$("#SbMessage").delay(1000).html("");
				});
			}
	);
}